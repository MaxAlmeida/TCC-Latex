...
\footnotetext[1]{\textit{OpenNebula} também provê uma interface \textit{web}, possui um módulo responsável por isso chamado \textit{Sunstone}}

\subsection{Automatização da Infraestrutura}

Para automatização bem como documentação via código dos procedimentos adotados para implantação da plataforma e nuvem, e dos serviços disponibilizados pelo LAPPIS (apresentados no apêndice \ref{sec:infraestrutura}), foi utilizada uma ferramenta de código aberto chamada \textit{Chake}\footnotemark[2]. 
